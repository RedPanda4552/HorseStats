...
public class HorseStatsUpdater {
     ...
     private HorseStatsUpdater.UpdateResult result = HorseStatsUpdater.UpdateResult.SUCCESS; // Used for determining the outcome of the update process
      ...
      */
     public HorseStatsUpdater(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
          this.plugin = plugin;
          this.type = type;
          ...
          try {
                this.url = new URL(HorseStatsUpdater.HOST + HorseStatsUpdater.QUERY + id);
      ...
      * @see UpdateResult
      */
     public HorseStatsUpdater.UpdateResult getResult() {
          this.waitForThread();
          return this.result;
     }
     ...
     private void saveFile(File folder, String file, String link) {
          ...
          try {
                ...
                final byte[] data = new byte[HorseStatsUpdater.BYTE_SIZE];
                int count;
                if (this.announce) {
                     this.plugin.getLogger().info("About to download a new update: " + this.versionName);
                }
                long downloaded = 0;
                while ((count = in.read(data, 0, HorseStatsUpdater.BYTE_SIZE)) != -1) {
                     downloaded += count;
                ...
                this.result = HorseStatsUpdater.UpdateResult.FAIL_DOWNLOAD;
          } finally {
                try {
                     if (in != null) {
                          in.close();
          ...
          try {
                ...
                while (e.hasMoreElements()) {
                     ...
                     if (entry.isDirectory()) {
                          ...
                          int b;
                          final byte buffer[] = new byte[HorseStatsUpdater.BYTE_SIZE];
                          final FileOutputStream fos = new FileOutputStream(destinationFilePath);
                          final BufferedOutputStream bos = new BufferedOutputStream(fos, HorseStatsUpdater.BYTE_SIZE);
                          while ((b = bis.read(buffer, 0, HorseStatsUpdater.BYTE_SIZE)) != -1) {
          ...
          if (this.type != UpdateType.NO_VERSION_CHECK) {
                ...
                if (title.split(delimiter).length == 2) {
                     ...
                     if (this.hasTag(localVersion) || !this.shouldUpdate(localVersion, remoteVersion)) {
                          ...
                          this.result = HorseStatsUpdater.UpdateResult.NO_UPDATE;
                          return false;
                     }
                } else {
                     ...
                     this.result = HorseStatsUpdater.UpdateResult.FAIL_NOVERSION;
                     return false;
                }
          }
          return true;
     ...
     private boolean hasTag(String version) {
          for (final String string : HorseStatsUpdater.NO_UPDATE_TAG) {
     ...
     private boolean read() {
          try {
                ...
                conn.addRequestProperty("User-Agent", HorseStatsUpdater.USER_AGENT);
                ...
                this.versionName = (String) ((JSONObject) array.get(array.size() - 1)).get(HorseStatsUpdater.TITLE_VALUE);
                this.versionLink = (String) ((JSONObject) array.get(array.size() - 1)).get(HorseStatsUpdater.LINK_VALUE);
                this.versionType = (String) ((JSONObject) array.get(array.size() - 1)).get(HorseStatsUpdater.TYPE_VALUE);
                this.versionGameVersion = (String) ((JSONObject) array.get(array.size() - 1)).get(HorseStatsUpdater.VERSION_VALUE);
     ...
     private class UpdateRunnable implements Runnable {
         ...
         @Override
          public void run() {
                if (HorseStatsUpdater.this.url != null) {
                     // Obtain the results of the project's file feed
                     if (HorseStatsUpdater.this.read()) {
                          if (HorseStatsUpdater.this.versionCheck(HorseStatsUpdater.this.versionName)) {
                                if ((HorseStatsUpdater.this.versionLink != null) && (HorseStatsUpdater.this.type != UpdateType.NO_DOWNLOAD)) {
                                     String name = HorseStatsUpdater.this.file.getName();
                                     // If it's a zip file, it shouldn't be downloaded as the plugin's name
                                     if (HorseStatsUpdater.this.versionLink.endsWith(".zip")) {
                                          final String[] split = HorseStatsUpdater.this.versionLink.split("/");
                                          name = split[split.length - 1];
                                     }
                                     HorseStatsUpdater.this.saveFile(new File(HorseStatsUpdater.this.plugin.getDataFolder().getParent(), HorseStatsUpdater.this.updateFolder), name, HorseStatsUpdater.this.versionLink);
                                } else {
                                     HorseStatsUpdater.this.result = UpdateResult.UPDATE_AVAILABLE;
                                }
                          }
                     }
                }
